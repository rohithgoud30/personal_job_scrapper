...
export async function evaluateJobDetail(payload: DetailPayload): Promise<{ accepted: boolean; reasoning: string }> {
  ...
  const systemPrompt = [
    ...
    'Return JSON { "accepted": boolean, "reasoning": string } and set accepted=true only if the stack aligns with the above. Experience is flexible: "1-5 years", "5+ years", "6 years", "6+ years", or "6-8 years" are acceptable. Reject only roles that explicitly demand more than six years (e.g., "7+ years", "8-10 years", "at least seven years"). The reasoning should be a concise one-liner explaining accept/reject.'
  ].join(' ');
