...
export async function evaluateJobDetail(payload: DetailPayload): Promise<{ accepted: boolean; reasoning: string }> {
  ...
  const systemPrompt = [
    ...
    'Return JSON { "accepted": boolean, "reasoning": string } and set accepted=true only if the stack aligns with the above AND the experience requirement is at least five years but below six years. Accept phrases like "5 years", "5+ years", "up to 5 years", "1-5 years". Reject anything that explicitly includes six or more years (e.g., "6 years", "6+ years", "5-7 years", "6-8 years", "7-10 years", "at least six years"). The reasoning should be a concise one-liner explaining accept/reject.'
  ].join(' ');
