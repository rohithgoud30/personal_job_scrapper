...
function getFirstKeyword(site: SiteConfig): string {
  const keywords = site.search.criteria.searchKeywords;
  if (Array.isArray(keywords)) {
    if (keywords.length === 0) {
      throw new Error(
        "No search keywords configured for site. Please add searchKeywords to your config.json."
      );
    }
    return keywords[0];
  }
...
async function runSmoke(): Promise<void> {
  ...
  const keyword = getFirstKeyword(site);
  ...
  if (cookieResult.clicked) {
    ...
    console.log(
      "[smoke] No cookie prompt interacted with (may not be present)."
    );
  }
  ...
  if (keywordInput) {
    ...
    await keywordInput.type(keyword, { delay: 10 }).catch(() => undefined);
  ...
  if (hitsHandle) {
    ...
    if (hits && hits.length) {
      ...
      hits.forEach((hit, index) => {
        console.log(
          `[${index + 1}] ${hit.title} | ${hit.location} | posted: ${
            hit.posted
          } | ${hit.url}`
        );
      });
    ...
    console.log(
      "[smoke] Route data not available; attempting DOM scrape for first page rows."
    );
    const debugCounts = await page.evaluate(() => {
      return {
        cards: document.querySelectorAll("ul.cards__list li.cards__item")
          .length,
    ...
    console.log(
      `[smoke] Debug counts: cards=${debugCounts.cards} anchors=${debugCounts.anchors} cardsList=${debugCounts.cardsList}`
    );
    ...
    if (cardCount === 0) {
      console.log(
        "[smoke] No result cards found in DOM; trying anchor scrape."
      );
      ...
      if (!anchors.length) {
        ...
        anchors.forEach((a: any, index: number) => {
          const url = a.href.startsWith("http")
            ? a.href
            : new URL(a.href, site.search.url).toString();
      ...
      for (let i = 0; i < count; i++) {
        ...
        console.log(
          `[${i + 1}] ${title} | posted: ${normalized || posted} | ${url}`
        );
      }
    }
  }
...
async function ensureDateSort(
  ...
  if (selectWait) {
    ...
    console.log(
      `[smoke] Found sort select#sortBy (value="${currentValue || "empty"}")`
    );
    if (currentValue !== target) {
      ...
      if (!changed || (Array.isArray(changed) && !changed.length)) {
        console.warn(
          "[smoke] Failed to change sort via selectOption; will try fallback toggle path."
        );
      } else {
        await page.waitForLoadState("networkidle").catch(() => undefined);
        return;
    ...
    console.warn(
      "[smoke] sort select#sortBy not found within timeout; falling back to toggle options."
    );
  }
  ...
  const toggle = await pickFirstAvailable(
    page,
    [
      sortToggle ?? "",
  ...
  const option = await pickFirstAvailable(
    page,
    [
      sortOption ?? "",
      'button:has-text("date")',
