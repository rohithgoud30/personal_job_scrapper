...
export async function runKforceSite(
  ...
  if (resumeSessionId) {
    ...
    if (!located) {
      console.warn(
        `[kforce] Session ${resumeSessionId} not found under ${path.join(output.root, site.host)}.`
      );
      return;
    }
  ...
  try {
    if (!resumeSessionId) {
      ...
      console.log(`[kforce] Session ${resumeSessionId} has no staged roles to evaluate.`);
      return;
    }
...
async function scrapeKeywordsInBatches(
  ...
  if (skipBatchDelay) {
    console.log('[kforce] Batch wait disabled; running keyword batches back-to-back.');
  }
