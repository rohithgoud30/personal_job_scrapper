...
export async function runKforceSite(
  site: SiteConfig,
  output: OutputConfig,
  options: RunOptions = {}
): Promise<void> {
  ...
  if (resumeSessionId) {
    ...
    if (!located) {
      console.warn(
        `[kforce] Session ${resumeSessionId} not found under ${path.join(
          output.root,
          site.host
        )}.`
      );
    ...
    console.log(
      `[kforce] Resuming AI-only run for session ${resumeSessionId} (date folder ${outputPaths.dateFolder}).`
    );
  } else {
    const dateLabel = getEasternDateLabel(runDate);
    if (isBackfill) {
      console.log(
        `[kforce] Backfill mode enabled. Using run date ${dateLabel}.`
      );
    } else {
  ...
  try {
    ...
    console.log(
      `[kforce][AI] Running title filter on ${stagedArray.length} staged roles...`
    );
    ...
    await writeSessionRoles(sessionPaths, filtered);
    console.log(
      `[kforce][AI] Title filter removed ${
...
async function scrapeKeywordInNewPage(
  ...
  try {
    ...
    console.log(
      `[kforce] Keyword "${keyword}": scraped ${rows.length}, staged ${added}`
    );
  } catch (error) {
...
async function scrapeKeyword(
  ...
  if ((await keywordInput.count()) === 0) {
    throw new Error(
      `Keyword input not found using selector ${keywordSelector}`
    );
  }
...
async function collectListingRows(
  ...
  while (true) {
    ...
    const loaded = await loadMoreWithRetry(
      page,
      nextSelector,
      cardSelector,
      processedCount,
...
async function evaluateDetailedJobs(
  ...
  for (let i = 0; i < roles.length; i++) {
    ...
    try {
      ...
      console.log(
        `[kforce][AI] Detail candidate #${i + 1}/${roles.length} "${
          role.title
  ...
  console.log(
    `[kforce][AI] Detail evaluation accepted ${accepted.length} roles out of ${roles.length}.`
  );
  return accepted;
}
...
async function ensureJobTypeFacet(
  ...
  const option = page
    .locator(facetSelector)
    .filter({ hasText: buildTextMatcher(targetLabel) })
    .first();
...
async function ensureNewestSort(
  ...
  if (currentLabelSelector) {
    ...
    if ((await currentLabel.count()) > 0) {
      const currentText = (
        await currentLabel.innerText().catch(() => "")
      ).trim();
...
async function extractJobRow(
  ...
  const locationText = selectors.locationText
    ? (
        await card
          .locator(selectors.locationText)
          .first()
          .innerText()
          .catch(() => "")
      ).trim()
