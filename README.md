...
- **Two-stage AI filtering**:
  1. After scraping completes, all staged titles/companies/locations/URLs are sent in one array to the OpenRouter model so it can drop irrelevant roles before further processing.
  2. Each remaining role is visited individually; the full job description is scored by AI, and only “accepted” postings are written to `new_jobs.csv`.
